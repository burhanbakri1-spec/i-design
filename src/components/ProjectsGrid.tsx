        ...
        <div className="w-[320px] shrink-0 bg-white p-6 flex flex-col">
          ...
          <div className="text-right mt-auto">
            ...
            <div className="pt-3 border-t border-black/10 mt-3">
              <p className="text-[10px] uppercase text-[#969696] mb-2">Share</p>
              <div className="flex gap-1.5 justify-end">
                <span className="flex items-center justify-center w-5 h-5 bg-black text-white cursor-pointer hover:opacity-70 transition-opacity">
          ...
          <button onClick={() => router.push(`/projects/${project.id}`)} className="mt-4 text-[10px] uppercase text-[#969696] hover:text-black transition-colors text-right">
            View full project →
          </button>
        </div>
        <div className="flex-1 min-w-0">
          <div className="h-[50vh] overflow-x-auto overflow-y-hidden no-scrollbar flex">
            <div className="relative w-[80vw] h-full shrink-0">
              <img src={project.images[0] || project.image} alt={project.title} className="w-full h-full object-cover" />
            </div>
            {detail.features.map((f, i) => (
              <div key={i} className="relative w-[80vw] h-full shrink-0">
...
function ProjectCard({ project, bigStyle, expanded, onToggle }: { project: Project; bigStyle?: boolean; expanded: boolean; onToggle: () => void }) {
  ...
  return (
    <div className="w-full">
      <div className="grid grid-cols-1 gap-3 w-full px-2 sm:px-0 lg:grid-cols-[290px_336px] lg:gap-x-[37px] text-left ml-[4cm]"
